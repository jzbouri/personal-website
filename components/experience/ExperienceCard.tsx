...
export default function ExperienceCard({ item }: ExperienceCardProps) {
  return (
    <Link href={item.companyUrl} target="_blank" rel="noreferrer noopener" aria-label={`Visit ${item.companyName} website`} className="group block h-full rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40">
      <article className="relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-[var(--surface-1)] p-4 transition-all duration-300 hover:border-white/20 hover:bg-[var(--surface-2)] sm:p-5 cursor-pointer">
      ...
      <div className="mt-3 space-y-1.5">
        ...
        {typeof item.companyDescription === "string" ? (
          <p
      ...
      <div className="mt-3 space-y-1.5">
        ...
        {typeof item.roleDescription === "string" ? (
          <p
            className="relative z-10 text-[15px] leading-[1.7] text-white/85 [&_a]:text-emerald-300 [&_a:hover]:text-emerald-200 [&_a]:underline [&_a]:decoration-emerald-400/50 [&_a]:underline-offset-2"
